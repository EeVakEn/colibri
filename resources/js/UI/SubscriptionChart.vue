...
onMounted(async () => {
    try {
        const response = await axios.get(route('subscription.getSubscriptionsByChannel'));
        ...
        const datasets = data.map(channel => ({
            ...
            data: allDates.map(date => {
                const subscription = channel.data.find(s => s.x === date.format('YYYY-MM-DD'));
                return subscription ? subscription.y : 0;
            }),
