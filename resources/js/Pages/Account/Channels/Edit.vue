...
const previewUrl = ref(props.channel.avatar);
...
const form = useForm({
    avatar: props.channel.avatar,
    name: props.channel.name,
    description: props.channel.description,
    is_free: props.channel.is_free,
    subscription_price: props.channel.subscription_price,
})
...
const submit = () => {
    ...
    for (const key in form.data()) {
        if (key === 'is_free') {
            formData.append(key, form.data()[key] ? '1' : '0');
        } else if (key === 'subscription_price' && form.data().is_free) {
            formData.append('subscription_price', 0);
        } else {
            formData.append(key, form.data()[key]);
        }
    }
    formData.append('_method', 'put');
    router.post(route('account.channels.update', [props.channel.id]), formData);
};
...
<template>
    ...
    <form @submit.prevent="submit" class="flex flex-col max-w-[600px] gap-4">
        ...
        <div v-if="!form.is_free">
            ...
            <div class="relative">
                <input id="subscription_price" placeholder="Price" type="text" class="input-field pl-6"
